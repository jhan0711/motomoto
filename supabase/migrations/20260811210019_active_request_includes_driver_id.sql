-- =============================================================================
-- Fase 14, paso 4: la solicitud dice tambien QUIEN es el conductor
-- =============================================================================
--
-- Una sola columna nueva: `driver_id`.
--
-- POR QUE HACE FALTA. El pasajero se suscribe en tiempo real a la posicion de su
-- conductor, y esa suscripcion lleva un filtro por `driver_id`. Sin el filtro
-- habria que escuchar la tabla entera y dejar que las politicas descarten lo
-- ajeno: funcionaria y seria seguro, pero pondria al servidor a evaluar la
-- politica de este pasajero contra cada movimiento de cada motorraton de la
-- flota. `driver_locations` es la tabla que mas se actualiza del sistema, y ese
-- coste se paga seis veces por minuto y por conductor en viaje.
--
-- NO ABRE NADA NUEVO. El pasajero con servicio asignado ya recibe el nombre, el
-- telefono, la calificacion, el numero de unidad y la placa de ese conductor
-- desde la Fase 13 (D167). Su identificador es menos revelador que cualquiera de
-- esos cinco, y sin el no puede pedir la posicion que las politicas ya le
-- permiten leer.
--
-- Se vuelve a escribir entera porque `create or replace` no admite cambiar las
-- columnas que devuelve una funcion. Esta version parte de la del paso 1, que es
-- la ULTIMA aplicada: comprobado con
--
