...
| D276 | **"Motorratón" pasa a "Motocarro" en todo lo que ve el usuario** | Pedido de la empresa, 2026-09-24 (punto 2 del plan del 2026-09-23): el vehículo real de Amalfi se llama motocarro. Cambiado en `src/`, `admin/src/`, `site/build.mjs`, `README.md` y `docs/` (textos legales, ficha de Play Store): pantallas, mensajes de error, panel, sitio público y comentarios del código. **La ruta del panel también pasó de `/motorratones` a `/motocarros`** (decisión del usuario; el panel es de uso interno, no hay enlaces externos que romper). Como los textos legales cambiaron de palabra, su "Última actualización" pasó a 24 de septiembre de 2026. **Base de datos:** una migración (`20260924120000_notify_arrived_motocarro.sql`) reemplaza `notify_driver_arrived`, el único texto de usuario que vive en la BD (la notificación "Tu motocarro llegó"). NO se tocaron los `raise exception` que dicen "motorraton" (16 funciones): son cadenas técnicas que la app y el panel traducen desde el `hint`, el usuario no las lee, y reescribir funciones de producción por eso es riesgo sin ganancia; tampoco los comentarios de migraciones ya aplicadas ni los comentarios de tabla. Verificado: `grep -ri motorrat` sobre `src`, `admin/src`, `site/build.mjs`, `README.md` y `docs` da cero. **Pendiente fuera del código:** las capturas de la ficha de Play Store que muestren "motorratón" hay que rehacerlas, y el texto de la ficha en Play Console se cambia a mano. |
| D277 | **El pasajero propone el precio y el conductor lo ve antes de aceptar** | Punto 4 del plan del 2026-09-23. **Decisión de la empresa: el piso es la tarifa mínima general** (`min(urban_fares.amount)`, hoy $4.000), NO la tarifa oficial de ese viaje: se puede ofertar por debajo de la oficial de una vereda, de noche o con más pasajeros. Tope: `max_offer_multiple` (5 por defecto, sin fila sembrada) veces la oficial, contra el error de un cero de más. **Base de datos** (`20260924130000_passenger_offers_price.sql`): `request_ride` recibe `p_offered_amount` opcional (sin él rige la tarifa oficial, así la app publicada vc9 sigue funcionando); `fare_amount` guarda lo acordado y `fare_trip_amount`/`fare_cargo_amount`/`fare_is_night`/`fare_is_rural`/`fare_reference` se CONSERVAN como la tarifa oficial de referencia (oficial total = viaje + carga, o la mayor en una encomienda; no hace falta columna nueva). Por eso se **quitó `rr_fare_matches_parts`**, que exigía total = suma. La comisión del 4% (punto 3) se calcula sobre `fare_amount`, o sea sobre lo acordado. Nuevos códigos `OFFER_BELOW_MINIMUM` / `OFFER_ABOVE_MAXIMUM`. `get_min_offer_amount()` da el piso a la pantalla. `list_driver_offers` ahora devuelve `fare_amount`, `fare_official_amount`, `service_type`, `parcel_description` y `cargo_summary` (antes el conductor aceptaba SIN ver precio, tipo de servicio ni carga) y pasó a `security definer` porque la carga solo es legible para el conductor ya vinculado. **App:** el resumen del pasajero tiene un campo "Tu oferta" precargado con la tarifa sugerida (`use-offer-price.ts`; si cambia la tarifa base, el campo vuelve a la sugerida); siempre se envía el valor que se ve; la `OfferCard` muestra valor (con la tarifa oficial al lado si difiere), tipo de servicio y carga; la pantalla del viaje del conductor y el detalle del panel dejan de mostrar el desglose cuando la oferta difiere de la oficial (no sumaría). Probado: `prueba_oferta_precio.sql` (11 comprobaciones). Se actualizaron `prueba_calculo_tarifa.sql` (42 y 44 ahora esperan que un total distinto de la suma ENTRE) y `prueba_solicitud_con_valor.sql` (firma nueva). **Pruebas SQL en rojo que NO son de este cambio:** `calificaciones`, `cancelaciones`, `detalle_historial`, `historial`, `notificaciones` (piden la cuenta fija `ad000000-…f1`, que ya no existe en producción) y `encomiendas` (elige al pasajero con `offset 1` y, sin esa cuenta, cae en otro que ya tiene un servicio vivo dentro de la transacción); no se compararon contra el estado anterior. **Sin verificar en dispositivo** (pantalla del pasajero y tarjeta del conductor): hace falta un APK nuevo. Sugerencia pendiente, no pedida: mostrar el valor también en el texto de la notificación push "Nueva solicitud". |
| D278 | **Libro de movimientos del conductor, comisión del 4% y bloqueo por saldo (punto 3, primera entrega; SIN Wompi)** | **Primera vez que la app maneja dinero: rompe la frontera "la plataforma no procesa pagos"** (recordado a la empresa antes de programar). Decisiones del usuario: (1) **todo se despliega APAGADO**: ajuste `driver_balance_enforced` = 0; al desplegar todos los conductores tienen saldo 0 y encenderlo de golpe bloquearía a Diego y a los demás que trabajan; con el interruptor apagado no se cobra ni se bloquea nada; (2) **el saldo puede quedar negativo**: si la comisión supera lo que queda se descuenta completa, el conductor queda bloqueado y la recarga paga primero la deuda. **Base de datos** (`20260924140000_driver_ledger_commission.sql`, `20260924150000_revoke_ledger_trigger_functions.sql`): tabla `driver_ledger` que SOLO se anade (disparador que impide UPDATE, DELETE y TRUNCATE hasta a las funciones `security definer`; una corrección es otra fila), con `kind` = `topup` / `commission` / `adjustment`, monto con signo, `ride_id`, `external_ref` único (para que una notificación repetida de Wompi no acredite dos veces), motivo obligatorio en ajustes; sin llave foránea a propósito, porque es registro contable y debe sobrevivir a la cuenta. El saldo es la SUMA del libro, nunca un número mutable. Ajustes nuevos: `commission_percent` = 4, `min_topup_amount` = 10000 (con rangos en `admin_set_setting`). **`complete_ride`** anota la comisión (`round(fare_amount × 4%)`, sobre el valor ACORDADO de D277) en la misma transacción que cierra el viaje, y si el saldo ya no alcanza apaga "disponible". **Bloqueo** (mínimo = 4% de la tarifa mínima = $160): `driver_can_work()` se consulta en tres sitios más una red de seguridad — el disparador de encendido (`block_unavailable_driver`), `accept_ride_offer`, `find_available_drivers` (el reparto ni le ofrece) — y encender el interruptor apaga a los conductores ya conectados sin saldo. Hint nuevo `INSUFFICIENT_BALANCE`. RPC: `get_my_balance`, `list_my_ledger` (conductor); `admin_adjust_driver_balance` (camino para revertir un descuento en disputa, siempre con motivo y en el registro de auditoría), `admin_list_driver_balances`, `admin_get_driver_ledger` (admin). Las funciones auxiliares (`driver_balance`, `driver_can_work`...) no las puede ejecutar nadie desde fuera. **Lo atrapó el barrido de seguridad:** las dos funciones de disparador nacieron ejecutables por `anon`; se revocó en la segunda migración. Probado: `prueba_saldo_conductor.sql` (25 comprobaciones, con el interruptor encendido dentro de una transacción que se deshace: producción sigue en 0 y con el libro vacío). **Pantallas (segunda parte):** la app del conductor tiene una tarjeta de saldo en su perfil (`BalanceCard`), que NO se pinta mientras el interruptor esté apagado, con el mensaje `INSUFFICIENT_BALANCE` en el catálogo de errores y sin botón de recargar (no hay pasarela: se le dice que se comunique con la empresa); el panel muestra el saldo en la ficha del conductor, un botón "Saldo" con el formulario de ajuste (a favor o en contra, motivo obligatorio) y la lista de movimientos, y los tres ajustes nuevos aparecen en Configuración. **Sin verificar en pantalla** (ni la app en la tablet ni el panel logueado). Pendiente de este punto: **la recarga por Wompi** (Edge Function con verificación de firma del webhook, llaves de sandbox de la empresa, política de privacidad y términos, declaración de Play Console). |
| D279 | **Recargas con Wompi, parte que no depende de las llaves (punto 3, segunda entrega)** | Investigación y diseño completos en `docs/operaciones/wompi-recargas.md`. La cuenta de Wompi de la empresa (comercio "AmalfiGoApp") ya existe y está en modo de pruebas (Sandbox) mientras Wompi valida los datos; con eso se puede construir y probar todo, y la producción espera su aprobación. **Base de datos** (`20260925120000_topup_intents.sql`): tabla `topup_intents` (la recarga nace ANTES del pago, con su referencia `AGA-` + 32 hex; el monto que se acredita sale de esta fila y nunca del aviso de pago) y cuatro funciones SOLO de `service_role`: `create_topup_intent` (valida conductor activo, mínimo, máximo de $1.000.000 y un freno de 10 intentos por hora), `credit_driver_topup` (idempotente con `for update`; un monto que no cuadra NO acredita y deja la intención en `review`), `fail_driver_topup` y `reverse_driver_topup` (Wompi puede anular una tarjeta DESPUÉS de aprobarla: se anade un ajuste en contra, el libro no se edita). Devuelven texto en vez de lanzar, para que la marca de `review` no se deshaga con la transacción. Probado: `prueba_recargas.sql` (21 comprobaciones; producción intacta). **Código** (`supabase/functions/`): `_shared/wompi.ts` con la firma del cobro y la verificación del aviso (concatena las propiedades que el propio aviso lista, más el timestamp y el secreto de eventos, SHA256, comparación en tiempo constante; rechaza si falta una propiedad), con 19 pruebas de jest (se añadió esa carpeta a `roots`); y dos Edge Functions, `wompi-webhook` (verifica firma, le pregunta a Wompi con la llave privada el estado real de la transacción y solo entonces llama a la función SQL) y `create-topup` (identifica al conductor por su JWT, nunca por el cuerpo, y firma el checkout en el servidor). Las Edge Functions quedan fuera de `tsc` y de ESLint (Deno). **Lo que NO está verificado:** las dos Edge Functions son pegamento que no se ha ejecutado nunca (no hay Deno en esta máquina y no están desplegadas); las pruebas de firma comprueban el algoritmo tal como lo describe la documentación, con el mismo entendimiento en el código y en la prueba, así que la confirmación real es el primer aviso de sandbox. **Falta:** desplegar las funciones, poner los secretos (`WOMPI_PUBLIC_KEY`, `WOMPI_PRIVATE_KEY`, `WOMPI_EVENTS_SECRET`, `WOMPI_INTEGRITY_SECRET`, `WOMPI_ENV`; los pone el usuario, no el asistente), configurar la URL de eventos en Wompi, el botón "Recargar" en la app, la página de retorno `amalfigo.app/recarga`, y decidir quién paga la comisión de la pasarela y la recarga mínima. |
| D280 | **Recargas con Wompi funcionando en sandbox (punto 3, tercera entrega)** | Las llaves de sandbox se cargaron como secretos de Supabase desde un archivo local ignorado por git que se borró después (nunca pasaron por el chat), las dos Edge Functions se desplegaron (`wompi-webhook` con `--no-verify-jwt`, `create-topup` con JWT) y la URL de eventos de sandbox se configuró en el panel de Wompi. Comprobado desde fuera: GET al webhook = 405, cuerpo no JSON = 400, aviso con firma falsa = 401 (lo que además prueba que los secretos están cargados), `create-topup` sin sesión = 401. **Prueba de punta a punta en la tablet Xiaomi (2026-09-25):** el conductor de QA (`conductor2.prueba`) recargó $20.000 con la tarjeta de prueba de Wompi `4242 4242 4242 4242`. En la base de datos: la intención quedó `approved` con el id de transacción de Wompi, hay UNA sola fila `topup` en el libro con ese mismo id como `external_ref`, y el saldo pasó de $1 (ajuste de QA para hacer visible la tarjeta) a $20.001. **Es la primera confirmación real de que la verificación de firma del aviso (concatenar las propiedades que el aviso lista + timestamp + secreto de eventos, SHA256) coincide con la de Wompi**: hasta entonces solo estaba probada contra nuestra propia lectura de la documentación. **App:** botón "Recargar" en la tarjeta de saldo del perfil del conductor (montos sugeridos + campo libre, pago en el navegador con `expo-web-browser`, y al volver relee el saldo hasta 5 veces sin afirmar que se acreditó si tarda); la tarjeta se ve si el cobro está encendido O si el conductor ya tiene movimientos. Página de retorno `amalfigo.app/recarga` (noindex). Correcciones que pidió el usuario al probar en la tablet: el campo "otro valor" salía diminuto (el diálogo centra su contenido y un campo sin ancho propio se encoge), se le puso formato de pesos (`$12.500`, `formatAmountInput`) también al campo "Tu oferta" del pasajero (D277), y ambos tienen ahora título. **Pendiente:** probar un pago RECHAZADO (tarjeta `4111 1111 1111 1111`) y uno ANULADO; decidir quién paga la comisión de Wompi y la recarga mínima (hoy $10.000 y la absorbe la empresa); producción: esperar la aprobación de la cuenta de Wompi, cargar las llaves de producción (`WOMPI_ENV=production`), configurar la URL de eventos de producción, actualizar política de privacidad y términos (que hoy dicen que no hay pagos) y la declaración de datos de Play Console, y solo entonces encender `driver_balance_enforced`. **Antes de abrir al público con `purge_qa_accounts.sql`:** el libro y `topup_intents` no tienen llave foránea a propósito, así que las filas de las cuentas QA borradas quedarán huérfanas; hay que decidir si se conservan o se limpian a mano (el libro no admite DELETE ni siquiera para el propietario: habría que desactivar el disparador dentro de una transacción). |
...
| 4 | Build de producción con EAS: keystore real (Play App Signing) + AAB + clave de FCM V1 para `co.amalfigo.app` | **HECHO.** 4a (2026-09-03). 4b: primer `.aab` (build `428cd943`, vc 3, 2026-09-04). Rehechos: `c1fea3ee` (vc 4); `64e30d94` (vc 5); `e858699f` (vc 6, descartado); `29f2db0f` (**vc 7**, TextureView + oscuro, promovido a prueba cerrada 2026-09-08). **`656bd5b6` (vc 8, 2026-09-08)** añade el resumen en `'content'` (`be65b86`). versionName sigue en 1.0.0. **Subido a la prueba cerrada y enviado a revisión la noche del 2026-09-08** |
...
**2. "Motorratón" -> "Motocarro" -HECHO (D276).** El vehículo real de Amalfi se llama
...
**4. El pasajero propone el precio -HECHO (D277).** Hoy `request_ride()` no recibe
